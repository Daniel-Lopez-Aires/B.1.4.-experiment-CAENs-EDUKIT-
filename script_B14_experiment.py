...
time[Cs], time[Co], time[Na], time[Ba] = 300,300,300,300


###Cs137
load = Read_hist_txt.Read_hist_txt_CAEN('Cs137_BGO_3Thre_5000_8_histo_histo.txt', time[Cs])  
...
###Co60
load = Read_hist_txt.Read_hist_txt_CAEN('Co60_BGO_3Thre_5000_8_histo_histo.txt', time[Co])  
...
###Na
load = Read_hist_txt.Read_hist_txt_CAEN('Na22_BGO_3Thre_5000_8_histo_histo.txt', time[Na]) 
...
###Ba
load = Read_hist_txt.Read_hist_txt_CAEN('Ba133_BGO_3Thre_5000_8_histo_histo.txt', time[Ba]) 
...
plt.plot(ADC_channel[:,Cs], counts_stored[:,Cs] )    
plt.plot(ADC_channel[:,Co], counts_stored[:,Co])
plt.plot(ADC_channel[:,Na], counts_stored[:,Na])  
plt.plot(ADC_channel[:,Ba], counts_stored[:,Ba])       
plt.legend(['Cs', 'Co', 'Na', 'Ba'], fontsize=10) 
plt.title("Spectras using BGO", fontsize=20)           #title
...
plt.grid(True) 
plt.xlim(0,25000)                       #limits of x axis
plt.savefig('counts.png', format='png')
...
index_min = 1300          #index that starts the peak (by looking the graph)
index_max = 1546          #index that ends the peak (by looking the graph)

counts_peak = counts_stored[index_min-1:index_max-1,Cs] 
ch_peak = ADC_channel[index_min-1:index_max-1,Cs]
...
plt.plot( ADC_channel[:,Cs], counts_stored[:,Cs], 'b.-')    
...
######Peak  1#####
index_min = 2200#          #index that starts the peak (by looking the graph)
...
ch_peak = ADC_channel[index_min-1:index_max-1,Co]
...
###Plot of the interval of the peak (indexes)
plt.figure(figsize=(10,6))                  #width, heigh 6.4*4.8 inches by default
plt.plot( ADC_channel[:,Co], counts_stored[:,Co], 'b.-')      
plt.plot( ch_peak , counts_peak, 'r.-')   
...
######Peak  2#####
index_min = 2550#          #index that starts the peak (by looking the graph)
index_max = 2800          #index that ends the peak (by looking the graph)
                #830, 1150 for the 26_5 measurements (9000 ch)
                #1100, 1400 for the 31/5 measurements
...
index_max = 1250          #index that ends the peak (by looking the graph)
...
ch_peak = ADC_channel[index_min-1:index_max-1,Na]
...
#Plot
plt.figure(figsize=(10,6))                  #width, heigh 6.4*4.8 inches by default
plt.plot( ADC_channel[:,Na], counts_stored[:,Na], 'b.-')     
plt.plot( ch_peak , counts_peak, 'r.-')   
...
index_min = 2450         #index that starts the peak (by looking the graph)
index_max = 2750          #index that ends the peak (by looking the graph)
                #2531, 3181 for the 26/5 measurements (9000ch)
                #2800 3500 for the 31/5 measurements (600s)
...
index_min = 700         #index that starts the peak (by looking the graph)
index_max =900          #index that ends the peak (by looking the graph)
                #2531, 3181 for the 26/5 measurements (9000ch)
                #2800 3500 for the 31/5 measurements (600s)

counts_peak = counts_stored[index_min-1:index_max-1,Ba] 
ch_peak = ADC_channel[index_min-1:index_max-1,Ba]
...
#Plot
plt.figure(figsize=(10,6))                  #width, heigh 6.4*4.8 inches by default
plt.plot( ADC_channel[:,Ba], counts_stored[:,Ba], 'b.-')     
plt.plot( ch_peak , counts_peak, 'r.-')   
...
plt.savefig('R_vs_E.png', format='png')
...
plt.text(600,15, 'ln(R(E)) = {0:1.3f} ln(E) + {1:1.3f} ; r = {2:1.3f}'
         .format(fit_lin_exp['Slope'],fit_lin_exp['Intercept'],fit_lin_exp['r']), fontsize=14)    #first 2 arguments are x,y position.
plt.ylim(0,70)                            #limits of y axis
plt.savefig('R_vs_E_CAENS_style.png', format='png')


#####Residuo, fit fallido de 1/sqrt(x)
# def fit_fun(A,x):
#     return A/np.sqrt(x)
    
# fit = scipy.optimize.curve_fit(fit_fun, Energy, R_stored, 2*np.sqrt(2* np.log(2)))       #fit
...
# plt.figure(figsize=(8,5))  #width, heigh 6.4*4.8 inches by default
# plt.errorbar(Energy, R_stored, delta_R_stored, fmt='.r', capsize = 5)
# plt.plot(Energy, fit_fun(opt_values, Energy), 'b*')           #fit
...
plt.tick_params(axis='both', labelsize=14)              #size of axis
plt.grid(True) 
plt.text(12e3,400, 'y(x) = {0:1.3f}x + {1:1.3f} ; r = {2:1.3f}'
